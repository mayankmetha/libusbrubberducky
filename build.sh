...
if [ $LIB_OS_NAME == "macos" ]; then
  ...
  clang++ -target x86_64-apple-macos11 -std=gnu++17 -o out/"$LIB_OS_NAME"/x86_64/usbrubberducky src/*.cpp
  chmod +x out/"$LIB_OS_NAME"/x86_64/usbrubberducky
  clang++ -target arm64-apple-macos11 -std=gnu++17 -o out/"$LIB_OS_NAME"/arm64/usbrubberducky src/*.cpp
